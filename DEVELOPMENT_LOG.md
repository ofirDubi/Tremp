# Tremp Development Log

This file contains the development journal entries from the original project. It documents the thought process, decisions, and progress made during development.
...
# The Algorithm
an "Isochrone" can be fetcehd - given a location and a time travel constraint, an Isochrone is a polygon which includes all the points the car can get to within that time limit.
This is pretty useful for me, as what i'd like to do is generate a path for the car, then get Isochrones for the car path (i can do this efficiently i guess),
...
If i want a quick POC, i can use the following -
    ...
    2. Get route (or let's say top 3 routes) with car for the drivers original destination
    ...
    4. Search for a route using "public transportation", only this will allow us to combine Tremps.

* An interesting thing to see is how are multi-modal route searching is done, and i can just copy this instead of theorising about stuff.
...
Note - in app, time_picker_spinner_pop_up has a problematic license which forces me to open-source my app.
...
* Ok i did it!
* Note that i came across a problem with wrap around of 24H clock... i dealt with it in a very ugly way, TODO actually deal with it.
